                    ...
                    uint8_t *ciphertext,  uint8_t *tag_out)
{
    ...
    /* Get the tag */
    if(1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, 16, tag_out))
        return -1;
                    ...
                    uint8_t *plaintext_out)
{
    ...
    if(!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, 16, (void *)tag_in))
        return -1;
    ...
    if(ret > 0) {
        ...
        /* Verify failed */
        memset(plaintext_out, 0, ciphertext_len); // unauthenticated plaintext must never escape
        return -1;
    }
}